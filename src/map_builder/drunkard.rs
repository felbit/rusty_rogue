...
// Tweak this number to change how the caverns look
// Higher number => more open caverns
const MAX_DISTANCE: usize = 400;

// If the desired floor count is not reached after the drunkard passed out
// (max distance traveled), start a new drunkard!
...
impl DrunkardsWalkArchitect {
    fn drunkard(&mut self, start: &Point, rng: &mut RandomNumberGenerator, map: &mut Map) {
        ...
        loop {
            ...
            if distance_staggered > MAX_DISTANCE {
                break;
            }
        }
    }
